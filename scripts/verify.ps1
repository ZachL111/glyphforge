...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-glyphforge-detail.ps1

node tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-glyphforge-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-glyphforge-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-glyphforge-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
