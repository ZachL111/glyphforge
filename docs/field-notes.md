...
`glyphforge` is easiest to review by starting with the fixture, not the prose.
...
The widest spread is between `geometry span` and `geometry span`, so those are the first two cases I would preserve during a refactor.
